...
int main(void)
{
	...
	for(;;) {
		...
		/**** Non Beacon setting for device ****/
		SPI_send16((((0x00<<1)+1)<<8)+0x05); // No Automatic Acknowledgement | No PAN Coordinator | No Coordinator | Packet error | promiscuous
		...
		SPI_send16((((0x31<<1))<<8)+0x08); // Read RX Int
		SPI_send16((((0x39<<1)+1)<<8)+0x02); // Disable package reception
		
		SPI_send24((((0x300<<1))<<12)+ 0x01 +(1<<23)); // RXFIFO Header 
		SPI_send24((((0x301<<1))<<12)+ 0x01 +(1<<23)); // RXFIFO FrmSize 
		SPI_send24((((0x302<<1))<<12)+ 0xA5 +(1<<23)); // RXFIFO HDR 
		SPI_send24((((0x303<<1))<<12)+ 0x0A +(1<<23)); // RXFIFO FRM

		SPI_send16((((0x39<<1)+1)<<8)+0x00); // Disable package reception
